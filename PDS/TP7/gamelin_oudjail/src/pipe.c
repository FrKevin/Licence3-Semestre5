	    ...
	    case 0:
	      if (i > 0) {
	        if(verbose) {
	          printf("Child : Close pipe (i -1) : %d in WRITE \n", (int)i-1);
						printf("Child : Redirection pipe (%d) READ to STDIN \n", (int)i-1);
	      ...
	      if(verbose) {
	        printf("Child : Close pipe i : %d in READ \n", (int)i);
					printf("Child : Redirection pipe (%d) WRITE to STDOUT \n", (int)i);
	  ...
	  case 0:
	    if (verbose) {
	      printf("End Work Child : Close last pipe (%d) in WRITE \n", (int)last_pipe);
				printf("End Work Child : Redirection pipe (%d) READ to STDIN \n", (int)last_pipe);
...
static void wait_all_child(pid_t *t_pid, int nbcmd){
	size_t i;
	send_verbose_message("Waiting child ... ");
	for(i = 0; i < nbcmd; i++) {
	  waitfg(t_pid[i]);
	}
	send_verbose_message("End Waiting child !");
}
